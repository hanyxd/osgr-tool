...
public final class MisclickOptimizer extends Module implements ItemUseListener, AttackListener, BlockBreakingListener {
	...
	private final BooleanSetting antiCharge = new BooleanSetting(EncryptedString.of("Anti Glowstone Charge"), false)
			.setDescription(EncryptedString.of("Prevents all charging of glowstone anchors"));

	// --- NEW SETTING ---
	private final BooleanSetting antiExplode = new BooleanSetting(EncryptedString.of("Anti Anchor Explode"), false)
			.setDescription(EncryptedString.of("Prevents you from interacting (exploding) a charged anchor"));
	// -------------------
	...
	private final BooleanSetting glowstoneOnGlowstone = new BooleanSetting(EncryptedString.of("Glowstone on Glowstone"), false)
			.setDescription(EncryptedString.of("Makes it so you can't place glowstone on/next to another glowstone"));
	...
	public MisclickOptimizer() {
		...
		addSettings(doubleGlowstone, glowstoneMisplace, antiCharge, antiExplode, anchorOnAnchor, glowstoneOnGlowstone, obiPunch, echestClick);
	}
	...
	public void onItemUse(ItemUseEvent event) {
		if (mc.crosshairTarget instanceof BlockHitResult hit) {
			...
			// --- FIX: Anti Anchor Explode ---
			// Prevent interaction IF:
			// 1. Setting is ON
			// 2. It IS a Respawn Anchor
			// 3. It IS charged
			// 4. We are NOT holding Glowstone (standard check)
			// 5. We are NOT holding a Respawn Anchor (NEW EXCEPTION: allows air-place/double-click strat)
			if (antiExplode.getValue()
					&& BlockUtils.isBlock(hit.getBlockPos(), Blocks.RESPAWN_ANCHOR)
			...
			if (BlockUtils.isBlock(hit.getBlockPos(), Blocks.GLOWSTONE) && glowstoneOnGlowstone.getValue() && mc.player.isHolding(Items.GLOWSTONE))
				event.cancel();
