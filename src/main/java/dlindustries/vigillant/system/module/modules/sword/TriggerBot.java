...
public final class TriggerBot extends Module implements TickListener, AttackListener {
    ...
    private final BooleanSetting checkShield = new BooleanSetting(EncryptedString.of("Check Shield"), false)
            .setDescription(EncryptedString.of("Syncs with Shield Disabler for shield breaking"));
    ...
    public void onTick() {
        try {
            ...
            // --- CLICK LOGIC ---
            if (isMace) {
                // Mace requires Right-Click to trigger
                if (GLFW.glfwGetMouseButton(mc.getWindow().getHandle(), GLFW.GLFW_MOUSE_BUTTON_RIGHT) != GLFW.GLFW_PRESS) {
                    return;
                }
            } else {
                // Other weapons rely on the On Left Click setting
                if (onLeftClick.getValue() && GLFW.glfwGetMouseButton(mc.getWindow().getHandle(), GLFW.GLFW_MOUSE_BUTTON_LEFT) != GLFW.GLFW_PRESS) {
                    return;
                }
            }

            // --- OFFHAND/BLOCKING CHECK ---
            // We bypass this check if using a Mace so right-clicking doesn't get blocked by offhand items
            if (!isMace && ((mc.player.getOffHandStack().getItem().getComponents().contains(DataComponentTypes.FOOD) ||
    ...
    public void onAttack(AttackEvent event) {
        // --- EVENT CANCELLER FIX ---
        // Do not cancel the event if we are in the middle of executing our Mace Attack
        if (isMaceAttack) {
            return;
        }
