...
public final class AutoInventoryTotem extends Module implements TickListener {
	...
	@Override
	public void onTick() {
		// FIX: Pause execution if UtilitySwapper is actively managing the inventory
		if (UtilitySwapper.isSwapping) return;
		...
		if (clock <= 0) {
			final int syncId = ((InventoryScreen) mc.currentScreen).getScreenHandler().syncId;
			final boolean needOffhand = inventory.offHand.get(0).getItem() != Items.TOTEM_OF_UNDYING;
			boolean needHotbar = false;
			if (hotbar.getValue()) {
				ItemStack mainHand = mc.player.getMainHandStack();
				needHotbar = mainHand.isEmpty() || forceTotem.getValue() && mainHand.getItem() != Items.TOTEM_OF_UNDYING;
			}

			int offhandSourceSlot = -1;
			int hotbarSourceSlot = -1;

			if (needOffhand) {
				offhandSourceSlot = mode.isMode(Mode.Blatant) ? InventoryUtils.findTotemSlot() : InventoryUtils.findRandomTotemSlot();
			}

			if (needHotbar) {
				hotbarSourceSlot = mode.isMode(Mode.Blatant)
						? findTotemSlotExcluding(offhandSourceSlot)
						: findRandomTotemSlotExcluding(offhandSourceSlot);
				if (hotbarSourceSlot == -1) {
					hotbarSourceSlot = mode.isMode(Mode.Blatant) ? InventoryUtils.findTotemSlot() : InventoryUtils.findRandomTotemSlot();
				}
			}

			if (needOffhand && offhandSourceSlot != -1) {
				mc.interactionManager.clickSlot(syncId, offhandSourceSlot, 40, SlotActionType.SWAP, mc.player);
			}

			if (needHotbar && hotbarSourceSlot != -1) {
				mc.interactionManager.clickSlot(syncId, hotbarSourceSlot, inventory.selectedSlot, SlotActionType.SWAP, mc.player);
			}
	...
	private int findTotemSlotExcluding(int excludedSlot) {
		PlayerInventory inv = mc.player.getInventory();
		for (int i = 9; i < 36; i++) {
			if (i == excludedSlot) continue;
			if (inv.main.get(i).getItem() == Items.TOTEM_OF_UNDYING) return i;
		}
		return -1;
	}

	private int findRandomTotemSlotExcluding(int excludedSlot) {
		PlayerInventory inv = mc.player.getInventory();
		int found = 0;
		for (int i = 9; i < 36; i++) {
			if (i == excludedSlot) continue;
			if (inv.main.get(i).getItem() == Items.TOTEM_OF_UNDYING) found++;
		}
		if (found == 0) return -1;
		int pick = (int) (Math.random() * found);
		for (int i = 9; i < 36; i++) {
			if (i == excludedSlot) continue;
